...
public sealed class AccountRepositoryFake : IAccountRepository
{
    ...
    public async Task<IAccount> Find(AccountId accountId, string userId)
    {
        Account? account = _context
           .Accounts
           .Where(e => e.UserId == userId && e.AccountId.Equals(accountId))
           .Select(e => e)
           .SingleOrDefault();
    ...
    public async Task<IList<Account>> GetAccounts(string userId)
    {
